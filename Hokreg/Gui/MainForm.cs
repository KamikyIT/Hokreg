...
namespace Uniso.InStat
{
    public partial class MainForm : Form
    {
        ...
        private void FillRow(Game.Marker mk)
        {
            ...
            if (action == Uniso.InStat.Game.Hockey.ActionEnum._18_05)
        ...
        private void InsertFine(Game.Marker mk, Player player, Half _half, int _second, bool withoutdelete)
        {
            ...
            {                
                ...
                if (mki.Player1 != null)
                {
                    ...
                    {
                        var place = tm.Tactics[0].Places.First(o => o.Player != null && o.Player.Id == mki.Player1.Id);
        ...
        private void UpdateUIThread()
        {
            ...
            var video_load = (vlcStreamPlayer1 != null && vlcStreamPlayer1.Mode != Players.StreamPlayer.PlayerMode.Stop) || HockeyIce.Role == HockeyIce.RoleEnum.Online;
            ...
            //Adding buttons
            //Помеха
            button200900.Enabled = mode_keys || mode_keys_add; //mode_keys_add;
            FormatAddButton(button200900, edit != null && edit.flag_hitch);
        ...
        private void RegisterBegin(int action_id, int action_type, int win)
        {
            try
            {
                ...
                if (HockeyIce.Role != HockeyIce.RoleEnum.Online || (HockeyIce.Role == HockeyIce.RoleEnum.Online && !Options.G.Game_NoStopVideoInOnline))   
                    vlcStreamPlayer1.Mode = Players.StreamPlayer.PlayerMode.Pause;

                Log.Write("CREATE NEW 2");
                ...
                lock (Game.editMarker)
                {
                    ...
                    Log.Write("CREATE NEW 4");

                    ProcessingMarker(Game.editMarker.G);
                }

                Log.Write("CREATE NEW 5");
            }
            catch (Game.GameBase.InsertMarkerException imex)
            {
                lock (Game.editMarker)
            ...
            try
                {
                    ...
                    if (player.Team.Tactics[0].Places.Any(o => (o.Amplua.Id == 1 && player.IsGk) || (o.Amplua.Id > 1 && !player.IsGk) && o.Player == null))
                    {
                        ShowStatus("", 0);
                        
                        var mkm = mklist[0];

                        var place = player.Team.Tactics[0].Places.First(o => (o.Amplua.Id == 1 && player.IsGk) || (o.Amplua.Id > 1 && !player.IsGk) && o.Player == null);
                        var mkc1 = new Game.Marker(Game, 14, place.GetCode()) { Player1 = player, Player2 = null, Half = Half, TimeVideo = video_second };
                        Log.Write(String.Format("RESTORE PLAYER {0}", player));
                        Game.Insert(mkc1);
                    }
                    else
                    {
        ...
        private void MainForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            timer1.Stop();
            if (vlcStreamPlayer1 != null)
                vlcStreamPlayer1.Mode = Players.StreamPlayer.PlayerMode.Stop;
        ...
        public void MainForm_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyData == Options.G.Hotkey_PauseResume)
            {
                if (vlcStreamPlayer1.Mode != StreamPlayer.PlayerMode.Stop)
                {
                    vlcStreamPlayer1.Mode =
                        vlcStreamPlayer1.Mode == Players.StreamPlayer.PlayerMode.Play
                        ? Players.StreamPlayer.PlayerMode.Pause
                        : Players.StreamPlayer.PlayerMode.Play;
                }
            ...
            var video_load = vlcStreamPlayer1 != null && vlcStreamPlayer1.Mode != Players.StreamPlayer.PlayerMode.Stop;
        ...
        void vlcStreamPlayer1_DownloadSceneResult(object sender, Uniso.InStat.Players.StreamPlayer.DownloadSceneResultEventAgrs e)
        {
            try
            {
                sync.Execute(() =>
        ...
        void vlcStreamPlayer1_DownloadPlaylistResult(object sender, Uniso.InStat.Players.StreamPlayer.DownloadPlaylistResultEventAgrs e)
        {
            try
            {
                sync.Execute(() =>
        ...
        private void vlcStreamPlayer1_PositionChanged(object sender, Players.StreamPlayer.PositionEventArgs e)
        {
            sync.Execute(() =>
                {
                    macTrackBar1.Maximum = vlcStreamPlayer1.DurationTotal;
        ...
        private void vlcStreamPlayer1_MediaOpened(object sender, EventArgs e)
        {
            sync.Execute(() =>
            {
                if (vlcStreamPlayer1.Mode == Players.StreamPlayer.PlayerMode.Play)
                {
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            if (prevmk.Any(o => o.Compare(4, new int[] { 1, 2, 3, 4, 5 }) || o.Compare(8, 1)))
            {
                ...
                var form = new Gui.GoalPointForm(Game, copy);
                form.ShowDialog();
